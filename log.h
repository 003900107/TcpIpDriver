...
//		CLogger();
	...
	public:
		//初始化log
		void initLogger(EnumLogLevel nLogLevel = EnumLogLevel::LogLevel_Info, const std::string strLogPath = "", const std::string strLogName = "");
		//写严重错误信息
	...
	private:
		...
		std::string GetTime();
		//设置新log文件名
		std::string GetNewFileName(std::string strLogName = "");
		//文件全路径得到文件名
