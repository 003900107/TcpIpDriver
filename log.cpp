...
namespace LOGGER
{
		...
		m_strLogName(strLogName)
	{
		...
		//创建日志文件
		m_strLogFilePath = GetNewFileName(m_strLogName);
/*
...
//
//	void CLogger::initLogger(EnumLogLevel nLogLevel, const std::string strLogPath, const std::string strLogName)
//	{
//		//初始化
//		m_pFileStream = NULL;
//		if (m_strLogPath.empty())
...
//
	...
	//设置新log文件名
	string CLogger::GetNewFileName(const string strLogName)
	{
		...
		//日志的名称如：201601012130.log
		string strNewName = FormatString("%04d%02d%02d_%02d%02d%02d.log", tm1.tm_year + 1900, tm1.tm_mon + 1, tm1.tm_mday, tm1.tm_hour, tm1.tm_min, tm1.tm_sec);
		
		if (strLogName.empty())
			strLogFilePath = m_strLogPath + strNewName;
		else
			strLogFilePath = m_strLogPath + strLogName + "_" + strNewName;
		
		//string strLogFilePath = m_strLogPath.append(m_strLogName);

		return strLogFilePath;
	}

	//判断日志时间是否超过10天
	bool CLogger::CheckFileTime()
	{
		time_t curTime;
		time(&curTime);
		tm tm1;
		...
		if ((tm1.tm_mday - m_CreatTime.tm_mday) > LOG_CYCLE)
		//if (((tm1.tm_min*60+tm1.tm_sec) - (m_CreatTime.tm_min*60+m_CreatTime.tm_sec)) > LOG_CYCLE)
			return true;
		else
			return false;
	}
